...
class EMNISTLines(BaseDataModule):
    ...
    def add_to_argparse(parser):
        ...
        parser.add_argument(
            ...
            help=(f"Min overlap between characters in a line, " f"between 0 and 1. Default is {DEFAULT_MIN_OVERLAP}"),
        )
        parser.add_argument(
            "--max_overlap",
            type=float,
            default=DEFAULT_MAX_OVERLAP,
            help=(f"Max overlap between characters in a line, " f"between 0 and 1. Default is {DEFAULT_MAX_OVERLAP}"),
        )
    ...
    def data_filename(self):
        return PROCESSED_DATA_DIRNAME / (
            f"ml_{self.max_length}_o{self.min_overlap:f}_{self.max_overlap:f}_"
            f"ntr{self.num_train}_ntv{self.num_val}_nte{self.num_test}_"
            f"{self.with_start_end_tokens}.h5"
        )
    ...
    def __repr__(self) -> str:
        ...
        dataset_sizes = f"{len(self.data_train)}, {len(self.data_val)}, {len(self.data_test)}"
        batch_x_stats = (
            x.shape,
            x.dtype,
            x.min().item(),
        ...
        data = (
            f"Train/val/test sizes: {dataset_sizes}\n"
